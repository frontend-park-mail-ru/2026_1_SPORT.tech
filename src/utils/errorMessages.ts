...
const ERROR_MESSAGE_MAP: Array<[RegExp, string]> = [
  ...
  [/invalid credentials|invalid credential|wrong password/i, 'Неверный email или пароль.'],
  [/user not found/i, 'Пользователь не найден.'],
  [/email already (exists|taken)/i, 'Этот email уже занят.'],
  [/username already (exists|taken)/i, 'Это имя пользователя уже занято.'],
  [/user already exists/i, 'Пользователь с таким email или именем уже существует.'],
